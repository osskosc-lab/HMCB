...
def build_report(
    ...
    lines = [
        ...
        f"**{overall}**",
        "",
        f"- Scorable clustered alerts: {int(summary['alerts'])}",
        f"- Right-censored recent alerts: {int(summary['censored_alerts'])}",
        ...
        f"- F1: {summary['f1']:.3f}",
        f"- False-positive rate: {summary['fpr']:.4f}",
        f"- Alerts per year: {summary['alerts_per_year']:.3f}",
        f"- False alarms per year: {summary['false_alarms_per_year']:.3f}",
        ...
        f"- Median lead time for captured events: {summary['median_lead_days']:.1f} trading days",
        f"- Mean lead time for captured events: {summary['mean_lead_days']:.1f} trading days",
        ...
        "",
        "## Continuous discrimination and VIX benchmark",
        "",
        f"- CORENO continuous-score ROC-AUC: {summary['coreno_roc_auc']:.3f}",
        f"- VIX ROC-AUC: {summary['vix_roc_auc']:.3f}",
        f"- CORENO minus VIX AUC: {summary['auc_difference_vs_vix']:.3f}",
        f"- VIX alert precision: {summary['vix_precision']:.3f}",
        f"- VIX alert recall: {summary['vix_recall']:.3f}",
        f"- VIX alert F1: {summary['vix_f1']:.3f}",
        f"- VIX false positives: {int(summary['vix_fp'])}",
        "",
        "ROC-AUC diagnostics are secondary and do not alter the pre-registered five-gate verdict.",
        "",
